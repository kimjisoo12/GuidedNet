...
import logging
from utils.four_metrics_amos import calculate_metrics
...
def test_calculate_metric():
    ...
    imdir = "infer/data/amos"     # test data
    snapshot_path = "infer/checkpoints/amos/"  + str(FLAGS.ratio) + '/' # Checkpoints
    output2 = 'infer/predict/amos/'     # prediction
    path1 = output2
    path2 = "infer/label/amos/"   # test label
    # log
    logging.basicConfig(filename= str(FLAGS.ratio) + '_amos_guidedNet.log',
    ...
    for file_name in file_list:
        ...
        for file_name in file_list:
            ...
            organ11, organ12, organ13, organ14, organ15, avg_jaccard = calculate_metrics(path1, path2)
            ...
            logging.info('organ13:{}'.format(organ14))
            logging.info('organ13:{}'.format(organ15))
            ...
            avg_dice14.append(organ14)
            avg_dice15.append(organ15)
        ...
        logging.info('Organ 14 mean:{}'.format(np.mean(avg_dice14)))
        logging.info('Organ 14 std:{}'.format(np.std(avg_dice14)))

        logging.info('Organ 15 mean:{}'.format(np.mean(avg_dice15)))
        logging.info('Organ 15 std:{}'.format(np.std(avg_dice15)))
        ...
        organ14 = round(np.mean(avg_dice14), 2)
        organ15 = round(np.mean(avg_dice15), 2)
        avg_jaccard = round(np.mean(avg_jaccard_final), 2)

        logging.info('$' + str(organ6) + '$&$' + str(organ7) + '$&$' + str(organ1) + '$&$' + str(organ3) + '$&$' + str(
            organ2) + '$&$' + str(organ8) + '$&$' + str(organ14) + '$&$' + str(organ9)
                     + '$&$' + str(organ10) + '$&$' + str(organ13) + '$&$' + str(organ15) + '$&$' + str(
            organ4) + '$&$' + str(organ5) + '$&$' + str(organ11) + '$&$' + str(organ12)
                     + '$&$' + str(avg_dice) + '\pm' + str(std_dice_final) + '$&$' + str(avg_jaccard) + '\pm' + str(
                std_jaccard_final) + '$')
