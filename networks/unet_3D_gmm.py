...
class unet_gmm(nn.Module):

    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True):
        super(unet_gmm, self).__init__()
...
if __name__ == '__main__':
    ...
    model = unet_gmm(n_channels=1, n_classes=2)
    input = torch.randn(4, 1, 112, 112, 80)
    flops, params = profile(model, inputs=(input,))
    print(flops, params)
    macs, params = clever_format([flops, params], "%.3f")
    print(macs, params)
    print("unet_gmm have {} paramerters in total".format(sum(x.numel() for x in model.parameters())))